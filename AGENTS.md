...
- `src/ProseMirrorTester.ts` — the public class and its `Options`. Deliberately thin: it constructs an `EditorView` mounted on a real jsdom element, installs the mocks below, and delegates `insertText` to `src/typing/` and `selectText` to `src/selection.ts`.
